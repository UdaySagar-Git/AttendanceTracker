...
const HolidaysArray = ({
  ...
  const handleDateChange = () => {
    ...
    const existingHoliday = holidayArray.find(
      (holiday) =>
        holiday.Date[0] === newHolidayDate.Date[0] &&
        holiday.Date[1] === newHolidayDate.Date[1] &&
        holiday.Date[2] === newHolidayDate.Date[2]
    );
    if (existingHoliday) {
      ...
      <div className="flex flex-wrap justify-center xl:justify-start">
        ...
        {!toggle ? (
          <button
            ...
            className="bg-gray-950 hover:bg-gray-800 text-white text-sm font-semibold py-1 px-3 mx-4 rounded-lg"
          >
            Edit
          </button>
        ) : (
      ...
      {toggle && (
        <div className="flex items-center mb-2 flex-wrap gap-1 sm:gap-2 justify-center  bg-white">
          <input
            ...
            className="px-2 sm:px-4 py-1 sm:py-2 border border-gray-300 rounded-md mr-2"
          />
          <button
            onClick={handleDateChange}
            className="bg-gray-950 hover:bg-gray-800 text-white text-sm font-semibold p-2 rounded-lg"
          >
            Add Holiday
          </button>
        </div>
      ...
      <div className="border shadow-lg border-black px-1 py-3 sm:p-3 rounded-xl  justify-center items-center max-h-[300px] overflow-y-scroll">
        <ul className="w-full px-2">
          {holidayArray?.length === 0 && (
            <p className="text-center">No Holidays</p>
          )}
          {holidayArray
            .sort((a, b) => new Date(a.dateTime) - new Date(b.dateTime))
            .filter((holiday) => new Date(holiday.dateTime) >= new Date())
            .map((holiday, index) => (
              <li
                key={index}
                className={`py-1 mb-0.5 px-2 rounded-md flex justify-between ${
                  new Date(holiday.dateTime) <= endDate &&
                  new Date(holiday.dateTime) >= startDate
                    ? "bg-gray-200"
                    : ""
                } }`}
              >
                <span
                  className={`w-[90px] ${
                    !holiday.isPublic ? "text-red-500" : "text-gray-500"
                  }`}
                >
                  {holiday.Date[0]}/{holiday.Date[1]}/{holiday.Date[2]}
                </span>
                {!toggle && (
                  <span className="font-semibold max-w-[200px] overflow-hidden whitespace-nowrap ">
                    {holiday.title}
                  </span>
                )}
                {toggle && (
                  <div className="flex justify-between">
                    ...
                    <button
                      ...
                      className=" bg-red-500 hover:bg-red-600 text-white text-sm font-semibold px-2 rounded-lg "
                    >
                      <MdDelete />
                    </button>
                  </div>
